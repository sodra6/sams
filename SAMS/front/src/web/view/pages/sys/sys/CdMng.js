...
import CodeDtlList from "../../../components/sys/sys/CodeDtlList";

import { bootstrap as BT } from "../../../../js/util/commonUtil";

function CdMng(props) {
  const [cdGrp, setCdGrp] = useState();
  const myGrid = useRef();
  ...
  const requestGridData = async () => {
    ...
    setCdGrp(grid.getItemByRowIndex(0).cdGrp);
  ...
  return (
    <div className="row">
      <div className="col-md-4 p-0 mb-4 float-left">
      ...
      <CodeDtlList cdGrp={cdGrp} />
    </div>
  );
}

export default CdMng;
